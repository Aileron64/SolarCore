    ...
    public class CoreBars : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            foreach (GameObject obj in objects)
            {
                ...
                line.Add(obj.GetComponent<LineRenderer>());
            }
        ...
        void MoveObjects()
        {
            ...
            for (int i = 0; i < objects.Count; i++)
            {
                ...
                //float distanceScaled = (derp - startingScales[i]).z; // get change in scale
                //Vector3 direction = objects[i].transform.TransformDirection(scaleAxis); // get movement direction, relative to object

                Vector3 targetPosision = startingPositions[i] * currentHeight;
                objects[i].transform.localPosition = Vector3.Lerp(objects[i].transform.localPosition, targetPosision, Time.smoothDeltaTime * lerpSpeed);


                line[i].SetPosition(0, objects[i].transform.position);
                line[i].SetPosition(1, new Vector3(1300, -820, 1000));
            }
        }
    }
}
